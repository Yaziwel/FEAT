...
def calc_metrics_for_dataset(ctx, metrics, real_data_path, fake_data_path, mirror, resolution, gpus, verbose, use_cache: bool, num_runs: int):
    ...
    args.dataset_kwargs = dnnlib.EasyDict(
        class_name='src.training.dataset.VideoFramesFolderDataset',
        path=real_data_path,
...
@click.command()
@click.pass_context
@click.option('--metrics', help='Comma-separated list or "none"', type=CommaSeparatedList(), default='fvd2048_16f,is50k,fid50k_full', show_default=True)
@click.option('--real_data_path', help='Dataset to evaluate metrics against (directory or zip) [default: same as training data]', default='/home/deeplearning/whh/Endora/data/Colonoscopic_frames', metavar='PATH')
@click.option('--fake_data_path', help='Generated images (directory or zip)', default='/home/deeplearning/whh/Endora/sample_videos/sample_videos_frames', metavar='PATH')
@click.option('--mirror', help='Should we mirror the real data?', type=bool, metavar='BOOL')
@click.option('--resolution', help='Resolution for the source dataset', type=int, default=128, metavar='INT')
