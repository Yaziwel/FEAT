...
def get_feature_detector(path, device=torch.device('cpu'), num_gpus=1, rank=0, verbose=False):
    assert 0 <= rank < num_gpus
    key = (path, device)
...
def compute_feature_stats_for_dataset(
    opts, detector_path, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64,
    ...
    if opts.cache:
        ...
        args = dict(dataset_kwargs=opts.dataset_kwargs, detector_path=detector_path, detector_kwargs=detector_kwargs,
                    stats_kwargs=stats_kwargs, feature_stats_cls=feature_stats_cls.__name__)
        md5 = hashlib.md5(repr(sorted(args.items())).encode('utf-8'))
        cache_tag = f'{dataset.name}-{get_feature_detector_name(detector_path)}-{md5.hexdigest()}'
    ...
    detector = get_feature_detector(path=detector_path, device=opts.device, num_gpus=opts.num_gpus, rank=opts.rank, verbose=progress.verbose)
...
def compute_feature_stats_for_generator(
    opts, detector_path, detector_kwargs, rel_lo=0, rel_hi=1, batch_size: int=16,
    ...
    detector = get_feature_detector(detector_path, device=opts.device, num_gpus=opts.num_gpus, rank=opts.rank, verbose=progress.verbose)
