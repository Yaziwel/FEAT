...
def compute_fid(opts, max_real, num_gen):
    detector_path = '/path/to/inception-2015-12-05.pt'
    assert detector_path != '/path/to/inception-2015-12-05.pt', 'You need to change the detector_path to the real path where the inception-2015-12-05.pt is.'
    detector_kwargs = dict(return_features=True) # Return raw features before the softmax layer.

    batch_size = NUM_FRAMES_IN_BATCH[opts.dataset_kwargs.resolution]

    mu_real, sigma_real = metric_utils.compute_feature_stats_for_dataset(
        opts=opts, detector_path=detector_path, detector_kwargs=detector_kwargs,
    ...
    mu_gen, sigma_gen = compute_gen_stats_fn(
        opts=gen_opts, detector_path=detector_path, detector_kwargs=detector_kwargs, batch_size=batch_size,
