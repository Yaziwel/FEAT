...
def compute_fvd(opts, max_real: int, num_gen: int, num_frames: int, subsample_factor: int=1):
    ...
    assert detector_path != '/path/to/i3d_torchscript.pt', 'You need to change the detector_path to the real path where the i3d_torchscript.pt is.'
    ...
    mu_real, sigma_real = metric_utils.compute_feature_stats_for_dataset(
        opts=opts, detector_path=detector_path, detector_kwargs=detector_kwargs, rel_lo=0, rel_hi=0,
    ...
    mu_gen, sigma_gen = compute_gen_stats_fn(
        opts=gen_opts, detector_path=detector_path, detector_kwargs=detector_kwargs, rel_lo=0, rel_hi=1, capture_mean_cov=True,
